...
that sends the plaintext, key to otp_enc_d to encrypt plaintext
...
int main(int argc, char *argv[]){
	...
	int input_plaintext_fd, input_key_fd;    /* Input file descriptors */
    ...
    int ret_plaintext_in, ret_key_in, ret_out, ret_in; //read file return values
	char* ciphertextAll = NULL; //all cipher text
	int i; //loop control variable
    ...
    input_plaintext_fd = open (argv [1], O_RDONLY);
    if (input_plaintext_fd == -1) {
    ...
    if (input_key_fd == -1) {
		...
		//close file
		close (input_plaintext_fd);
        exit(2);
    }


	//read plain text and key
	memset(plaintext, '\0', BUFF_SIZE); // Clear out the buffer array
	memset(key, '\0', BUFF_SIZE); // Clear out the buffer array

	while((ret_plaintext_in = read (input_plaintext_fd, &plaintext, BUFF_SIZE - 1)) > 0){			

		//ignore new line character
		if (plaintext[ret_plaintext_in - 1] == '\n')
		{
			plaintext[ret_plaintext_in - 1] = '\0';
			ret_plaintext_in--;			
		}else{
			//append with '\0'
			plaintext[ret_plaintext_in] = '\0';
		}

		ret_key_in = read (input_key_fd, &key, ret_plaintext_in);//read key
		if (ret_plaintext_in != ret_key_in)
		{
		...
		//printf("Debug: plaintext: %s length = %d\n", plaintext, ret_plaintext_in);
		//printf("Debug: key: %s length = %d\n", key, ret_key_in);
		
		//send plaintext to server to encrypt
		ret_out = sendData(plaintext, socketFD);

		if (ret_out != ret_plaintext_in)
		{
			perror ("Error: send plain text\n");
			//close files
			close (input_plaintext_fd);
			close (input_key_fd);
			exit(2);
		...
		//printf("Debug: sent plaintext: %s length = %d\n", plaintext, ret_out);
		...
		//receive cipher text
		ret_in = receiveData(ciphertext, socketFD);
		if (ret_out != ret_in)
		{
			perror ("Error: receive cipher key");
			//close files
			close (input_plaintext_fd);
			close (input_key_fd);
			exit(2);
		...
		//printf("Debug: got cihper: %s length = %d\n", ciphertext, ret_in);

		//print to standard output
		//printf("%s", ciphertext);
		if (ciphertextAll == NULL)
		{
			ciphertextAll = (char*)malloc((ret_in + 1) * sizeof(char));
			strcpy(ciphertextAll, ciphertext);
		}else{
			ciphertextAll = (char*)realloc(ciphertextAll, (strlen(ciphertextAll) + ret_in + 1) * sizeof(char));
			strcat(ciphertextAll, ciphertext);
		}		
